...
<div class="card shadow mb-4">
	<div class="card-body">
		...
		<form action="/admin/countries" method="post" accept-charset="utf-8" enctype="multipart/form-data">
			...
			<div class="form-group">
				<label for="author">{{ __('Валюта') }}</label>
				<input type="text" name="currency" class="form-control" required>
			</div>
			<button type="submit" class="btn btn-primary" value="0" name="btncreate">{{ __('Создать') }}</button>
			<button type="submit" class="btn btn-success" value="1" name="btncreate">{{ __('Создать еще') }}</button>
		</form>
	</div>
</div>
@endsection